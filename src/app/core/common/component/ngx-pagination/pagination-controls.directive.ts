...
export class PaginationControlsDirective {
  ...
  @Output() showChangeItem: EventEmitter<any> = new EventEmitter<any>();
  @Output() pageBoundsCorrection: EventEmitter<number> = new EventEmitter<number>();
  pages: Page[] = [];

  private changeSub: Subscription;
  private changePageShowSub: Subscription;
  ...
  constructor(private service: PaginationService,
    ...
    this.changePageShowSub = this.service.changePageShowItem
      .subscribe(items => {
        this.showChangeItem.emit(items);
      });
  }
  ...
  ngOnDestroy() {
    ...
    this.changePageShowSub.unsubscribe();
  }
  ...
  last() {
    this.checkValidId();
    let lastPage = this.pages[this.pages.length - 1].value;
  ...
  getItemsPerPage(): number {
    let inst = this.service.getInstance(this.id);
    let itemsPerPage = inst.totalItems - ((inst.currentPage - 1) * inst.itemsPerPage);
    if (itemsPerPage > inst.itemsPerPage) {
      return inst.itemsPerPage;
    }
    return itemsPerPage
  }
